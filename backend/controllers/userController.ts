...
const bookAppointment = async (req, res) => {
  try {
    ...
    const appointmentData = {
      ...
      slotDate,
      date: Date.now(),
      meetingRoom: `Prescripto-${Math.random().toString(36).substring(2, 10)}`,
    };
...
const addReview = async (req, res) => {
  try {
    ...
    const appointmentData = await appointmentModel.findById(appointmentId);
    if (!appointmentData || appointmentData.userId !== userId || !appointmentData.isCompleted) {
    ...
    const doctorReviews = await reviewModel.find({ docId });
    const totalReviews = doctorReviews.length;
    const averageRating = doctorReviews.reduce((acc, curr) => acc + curr.rating, 0) / totalReviews;
